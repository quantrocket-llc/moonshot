...
class PositionWeightMixin(object):
    """
    Mixin class with utilities for turning signals into weights.
    """
    def apply_equal_weights(self, signals, cap=1.0):
        """
    ...
    def apply_fixed_weights(self, signals, weight):
        """
        Applies the specified fixed weight to the signals.
        """
        return signals * weight

    def apply_fixed_weights_capped(self, signals, weight, cap=1.0):
        """
        Applies fixed weights, but if the sum of the weights exceeds the cap,
        applies equal weights.
        """
        equal_weighted = self.apply_equal_weights(signals, cap=cap)
        fixed_weighted = self.apply_fixed_weights(signals, weight)
    ...
    def apply_market_neutral_fixed_weights_capped(self, signals, weight, cap=1.0,
        ...
        long_weights = self.apply_fixed_weights_capped(long_signals, weight, cap=cap_per_side)
        short_weights = self.apply_fixed_weights_capped(short_signals, weight, cap=cap_per_side)
    ...
    def apply_capped_weights(self, weights, weight_cap=None, total_cap=1.0):
        """
        Proportionately reduces the weights if their sum exceeds the cap.
        """
        if weight_cap is not None:
            weights = weights.where(weights <= weight_cap, weight_cap)
            weights = weights.where(weights >= -weight_cap, -weight_cap)

        total_weights = weights.abs().sum(axis=1)
        total_weights = pd.DataFrame(dict((column, total_weights.copy()) for column in weights.columns),
                                     index=weights.index, columns=weights.columns)
    ...
    def apply_constant_volatility_weights(self, weights, price_panel,
                                          target_std, std_window):
        """
        Applies constant volatility weights to the weights, based on the
        standard deviation lookback window, to achieve the target annualized
        standard deviation.
        """
        target_weights = weights
        positions = self.get_positions(target_weights, price_panel)
        ...
        weights = target_weights * volatilty_weights
